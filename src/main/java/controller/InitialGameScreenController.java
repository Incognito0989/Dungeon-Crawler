...
import src.main.java.item.weapon.*;
...
public class InitialGameScreenController {
    ...
    @FXML private Label damageLabel1;
    @FXML private Label killsLabel1;
    ...
    public void onWasd(KeyEvent event) throws IOException {
        if (event.getCode() == KeyCode.B && potionDrop != null) {
            //button to pick up item closest to player
        ...
        if (event.getCode() == KeyCode.P && room.getMonster() != null) {
            //an extra button that can be used to attack monster
            double xPlayer = playerMove.getTranslateX();
            double yPlayer = playerMove.getTranslateY();
        ...
        if (!doorWest.isDisabled() && y <= 45 && x <= -315 && y >= -100) {
            if (!room.getConnectingRoom(Direction.valueOf("WEST")).getVisited()) {
                player.setRoomsVisited(player.getRoomsVisited() + 1);
            }
            ...
            if (!room.getConnectingRoom(Direction.valueOf("EAST")).getVisited()) {
                player.setRoomsVisited(player.getRoomsVisited() + 1);
            }
            ...
            if (!room.getConnectingRoom(Direction.valueOf("NORTH")).getVisited()) {
                player.setRoomsVisited(player.getRoomsVisited() + 1);
            }
            ...
            if (!room.getConnectingRoom(Direction.valueOf("SOUTH")).getVisited()) {
                player.setRoomsVisited(player.getRoomsVisited() + 1);
            }
    ...
    public void setRoom() {
        ...
        for (Direction d: Direction.values()) {
            int[] x = room.getConnectingRoom(d).getCoords();
            for (int i: x) {
                System.out.print(i + " ");
            }
            System.out.println();
        ...
        System.out.println("exit: " + exitRoom.getCoords()[0] + " : " + exitRoom.getCoords()[1]);
        exitCoord.setText("Exit: " + exitRoom.getCoords()[0] + " " + exitRoom.getCoords()[1]);
    }

    public void setRoom(Room room, ExitRoom exitRoom) {
        this.room = room;
        ...
        itemOne.setImage(new Image(player.getWeapon().getImage()));
        itemOne.setDisable(false);
        ImageView[] items = {itemOne, itemTwo, itemThree, itemFour, itemFive, itemSix};
        for (int i = 1; i < 6; i++) {
        ...
        loadNewRoom();
        changeInitialGold(player.getGold());
        setWeapon(player.getWeapon().getName());
    ...
    /**
     * set the starting room given an already created Maze
     * @param m pregenerated Maze to set the dungeon's starting room to
     */
    public void setRoom(Maze m) {
        itemOne.setImage(new Image(player.getWeapon().getImage()));
        itemOne.setDisable(false);
        itemOneDesc.setPromptText(player.getWeapon().getDescription());
        this.room = m.getStartingRoom();
        roomCoord.setText("Room: " + room.getCoords()[0] + " " + room.getCoords()[1]);
        exitRoom = new ExitRoom(new int[]{3, 3});
    ...
    public void onDoorClick(MouseEvent event) {
        ...
        if (!room.getConnectingRoom(Direction.valueOf(dir)).getVisited()) {
            player.setRoomsVisited(player.getRoomsVisited() + 1);
        }
    ...
    public void onExitDungeon(MouseEvent event) throws IOException {
        ...
        ((Label) root.lookup("#damageLabel1")).setText("Damage Dealt: " + player.getDamageDealt());
        ((Label) root.lookup("#killsLabel1")).setText("Kills: " + player.getMonstersKilled());
        ((Label) root.lookup("#roomsVisitedLabel1")).setText("Rooms Visited: "
                + player.getRoomsVisited());
        ((Label) root.lookup("#goldLabel1")).setText("Gold: " + player.getGold());
    }

    @FXML
    public void onChallengeRoomEnter(MouseEvent event) throws IOException {
        int cost = 75;
        if (player.getGold() >= cost) {
            player.setGold(player.getGold() - cost);
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/src/main/resources/"
                    + "screens/challengeRoomScreen.fxml"));
            Parent root = loader.load();
            ChallengeRoomController c = loader.getController();
            c.setScene(player, (ChallengeRoom) room, (ExitRoom) exitRoom);
            Stage stage = (Stage) ((Node) event.getSource()).getScene().getWindow();
            stage.getScene().setRoot(root);
        }
    }
    ...
    private void setAvailableMovementButtons() {
        ...
        if (room.getClass() == ChallengeRoom.class && !((ChallengeRoom) room).allMonstersDead()) {
    ...
    private void playerAttackHelper(Event event) throws IOException {
        ...
        if (monster.getHealth() <= 0) { //monster dies
            ...
            if (room.getCoords().equals(exitRoom.getCoords())) {
                ...
                try {
                    doorNorth.setVisible(false);
                    doorNorth.setDisable(true);
                } catch (Exception e) {

                }
                try {
                    doorEast.setVisible(false);
                    doorEast.setDisable(true);

                } catch (Exception e) {

                }
                try {
                    doorWest.setVisible(false);
                    doorWest.setDisable(true);
                } catch (Exception e) {

                }
                try {
                    doorSouth.setVisible(false);
                    doorSouth.setDisable(true);
                } catch (Exception e) {
     ...
     */
    private void drop() {
        if (room.getMonster().getItems() != null) {
            Potion p = (Potion) room.getMonster().getItems()[0];
            Weapon w = (Weapon) room.getMonster().getItems()[1];
            if (p != null) {
                room.setPotion(p);
                potionDrop.setVisible(true);
                potionDrop.setDisable(false);
                potionDrop.setTranslateX(100);
                potionDrop.setTranslateY(100);
                potionDrop.setImage(new Image(p.getPotionImage()));
            }
            if (w != null) {
                room.setWeapon(w);
                weaponDrop.setVisible(true);
                weaponDrop.setDisable(false);
                weaponDrop.setTranslateX(-100);
                weaponDrop.setTranslateY(-100);
                weaponDrop.setImage(new Image(w.getImage()));
            }
            return;
        }
        ...
        if (dum == dum2) { //drop potion
            ...
            System.out.println(room.getPotion().getPotionName());
        ...
        if (dum == dum2) { //drop weapon
            ...
            weaponDrop.setVisible(true);
            weaponDrop.setDisable(false);
            int randp = rand.nextInt(4); //bound is the number of weapons
            String str1 = new String();
            switch (randp) {
            case(0):
